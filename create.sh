...
LAYER="sicar:sicar_imoveis_ac"
ogr2ogr -skipfailures -f geojson "$DIR"/dml/sicar_imoveis_ac.json \
  --config GDAL_HTTP_UNSAFESSL YES WFS:"$BASIS_URL" $LAYER
ogr2ogr -f PostgreSQL \
  PG:"host=$PGHOST user=$PGUSER password=$PGPASS dbname=$PGDATABASE" \
  -nln sicar_imoveis_ac \
  "$DIR"/dml/sicar_imoveis_ac.json
rm "$DIR"/dml/sicar_imoveis_ac.json
